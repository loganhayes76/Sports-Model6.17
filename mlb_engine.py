...
def fetch_live_mlb_intel(date_str):
        ...
        for g in r['dates'][0]['games']:
            try:
                ...
                for side_key, abbr in [('away', a_abbr), ('home', h_abbr)]:
                    ...
                    pp = team_node.get('probablePitcher') or {}
                    p_name = pp.get('fullName', 'TBD')
                    p_id   = pp.get('id')
                    ph_node = pp.get('pitchHand') or {}
                    ph_code = ph_node.get('code', '')
                    if ph_code == 'L':
                        p_hand = 'LHP'
                    elif ph_code == 'R':
                        p_hand = 'RHP'
                    else:
                        p_hand = ''  # unknown — don't guess
                    pitchers[side_key] = {
                        'p_name':   p_name,
                        'p_hand':   p_hand,
                        'p_id':     p_id,
                ...
                if game_pk:
                    try:
                        ...
                        # Collect all batting-order player IDs for a single batch
                        # handedness lookup after processing both sides.
                        all_pids = []
                        side_data = {}  # side_key -> list of partial player dicts

                        for side_key in ('away', 'home'):
                            team_bs = bs_teams.get(side_key, {})
                            bo_ids = team_bs.get('battingOrder', [])
                            players_dict = team_bs.get('players', {})

                            side_players = []
                            for pid in bo_ids:
                                pnode = players_dict.get(f'ID{pid}', {})
                                person = pnode.get('person', {})
                                stats  = pnode.get('seasonStats', {}).get('batting', {})
                                pos_node = pnode.get('position', {})
                                side_players.append({
                                    'id':       pid,
                                    'name':     person.get('fullName', 'Unknown'),
                                    'number':   pnode.get('jerseyNumber', ''),
                                    'position': pos_node.get('abbreviation', ''),
                                    'avg':      stats.get('avg', '.000'),
                                    'bats':     '',   # filled after batch lookup
                                })
                                all_pids.append(pid)

                            side_data[side_key] = side_players

                            # Manager from coaches list
                            for coach in team_bs.get('coaches', []):
                                title = (coach.get('position', {}).get('name', '')
                                         or coach.get('position', {}).get('type', '')).lower()
                                if 'manager' in title:
                                    managers[side_key] = coach.get('person', {}).get('fullName', '')
                                    break

                        # Enrich pitcher jersey number and ERA from boxscore
                        for sk in ('away', 'home'):
                            pid = pitchers[sk].get('p_id')
                            if pid:
                                pnode = bs_teams.get(sk, {}).get('players', {}).get(f'ID{pid}', {})
                                pitchers[sk]['p_number'] = pnode.get('jerseyNumber', '')
                                pitchers[sk]['p_era'] = (
                                    pnode.get('seasonStats', {}).get('pitching', {}).get('era', '--') or '--'
                                )

                        # Batch-fetch handedness for all batting-order players.
                        # NOTE: the MLB people endpoint ignores nested subfields in
                        # the ?fields= filter, returning batSide as {}.  Omitting
                        # the fields param returns the full person object which
                        # includes batSide.code correctly.
                        bats_map = {}
                        if all_pids:
                            try:
                                ids_str = ','.join(str(pid) for pid in all_pids)
                                ph = requests.get(
                                    f"https://statsapi.mlb.com/api/v1/people"
                                    f"?personIds={ids_str}",
                                    timeout=8
                                ).json()
                                for person in ph.get('people', []):
                                    bats_map[str(person['id'])] = (
                                        (person.get('batSide') or {}).get('code', '') or ''
                                    )
                            except Exception:
                                pass

                        # Merge handedness and finalise lineups
                        for side_key in ('away', 'home'):
                            for p in side_data[side_key]:
                                p['bats'] = bats_map.get(str(p['id']), '')
                            lineups[side_key] = side_data[side_key]
                ...
                for side_key, abbr in [('away', a_abbr), ('home', h_abbr)]:
                    ...
                    intel[abbr] = {
                        ...
                        'p_id':     pitchers[side_key].get('p_id'),
                        'p_number': pitchers[side_key].get('p_number', ''),
                        'p_era':    pitchers[side_key].get('p_era', '--'),
                        'lineup':   status,
                        'players':  players,
                        'manager':  managers[side_key],
                        'opp':      opp,
    ...
    # Fallback: batch-fetch pitchHand for any pitcher whose hand is still unknown
    unknown_hand = {
        abbr: entry['p_id']
        for abbr, entry in intel.items()
        if entry.get('p_hand') == '' and entry.get('p_id')
    }
    if unknown_hand:
        try:
            ids_str = ','.join(str(pid) for pid in set(unknown_hand.values()))
            ph_resp = requests.get(
                f"https://statsapi.mlb.com/api/v1/people?personIds={ids_str}",
                timeout=8
            ).json()
            hand_by_id = {}
            for person in ph_resp.get('people', []):
                code = (person.get('pitchHand') or {}).get('code', '')
                if code:
                    hand_by_id[person['id']] = 'LHP' if code == 'L' else 'RHP'
            for abbr, pid in unknown_hand.items():
                if pid in hand_by_id:
                    intel[abbr]['p_hand'] = hand_by_id[pid]
        except Exception:
            pass
