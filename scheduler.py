...
def append_scheduler_log(
    job_name: str, success: bool, output: str = "", duration_s: float = 0
):
...
def run_mlb_props():
    """6 AM — Daily MLB Prop Slayer"""
    log.info("═══ [6AM] Daily MLB Prop Slayer ═══")
    run_script("analyze_props.py", "Daily MLB Prop Slayer (6AM)")


def run_mlb_batter_stats():
    """Refresh the per-game batter rate cache from the MLB Stats API (free, no key)."""
    run_script("mlb_batter_stats.py", "MLB Batter Stats Refresh")


def run_mlb_prop_db():
    """Generate mlb_prop_database.json (season stats for the Prop Matrix)."""
    run_script("generate_mlb_prop_database.py", "MLB Prop DB Refresh")


def job_mlb_props_afternoon():
    """2 PM — Afternoon MLB Props refresh (captures lines posted mid-morning)."""
    log.info("═══ [2PM] Afternoon MLB Props Refresh ═══")
    run_mlb_batter_stats()
    run_mlb_prop_db()
    run_script("update_mlb_props.py", "MLB Props Update (2PM)")
    run_script("analyze_props.py", "MLB Prop Slayer (2PM)")
...
def job_daily_auto_log():
    ...
    try:
        ...
        log.info(
            f"  Auto-logger: {result.get('ncaa', 0)} NCAA + {result.get('mlb', 0)} MLB = {total} plays logged"
        )
        append_scheduler_log(
            "Daily Auto-Logger",
            True,
...
def _schedule_todays_auto_log(sched):
    ...
    if aware_log_time <= now_et:
        log.info(
            f"  ⚡ Log time {naive_log_time.strftime('%I:%M %p')} ET already passed — running auto-log now."
        )
        job_daily_auto_log()
        return
    ...
    log.info(
        f"  📌 Daily auto-log scheduled for {naive_log_time.strftime('%I:%M %p ET')}"
    )
...
def main():
    ...
    scheduler.add_job(
        ...
        minute=0,
        id="nightly_stats",
        name="3AM Nightly Stat Scrapers",
        misfire_grace_time=1800,
    )

    # 6:00 AM ET — MLB Prop Slayer
    scheduler.add_job(
        run_mlb_props,
        "cron",
        hour=6,
        minute=0,
        id="mlb_prop_slayer",
        name="6AM Daily MLB Prop Slayer",
        misfire_grace_time=1800,
    )

    # 8:00 AM ET — Full morning refresh
    scheduler.add_job(
        job_morning_full_refresh,
        "cron",
        hour=8,
        minute=0,
        id="morning_refresh",
        name="8AM Morning Full Refresh",
        misfire_grace_time=1800,
    )

    # 7:30 AM ET — MLB prop database refresh (season stats for the Prop Matrix)
    scheduler.add_job(
        run_mlb_prop_db,
        "cron",
        hour=7,
        minute=30,
        id="mlb_prop_db_am",
        name="7:30AM MLB Prop DB Refresh",
        misfire_grace_time=900,
    )

    # 7:45 AM ET — MLB batter stats pre-fetch (before 8AM props run)
    scheduler.add_job(
        run_mlb_batter_stats,
        "cron",
        hour=7,
        minute=45,
        id="mlb_batter_stats_am",
        name="7:45AM MLB Batter Stats Refresh",
        misfire_grace_time=900,
    )
    ...
    scheduler.add_job(
        ...
        minute=0,
        id="auto_grader",
        name="9AM Auto-Grader",
        misfire_grace_time=1800,
    )

    # 1:30 PM ET — MLB prop database refresh (before 2PM props run)
    scheduler.add_job(
        run_mlb_prop_db,
        "cron",
        hour=13,
        minute=30,
        id="mlb_prop_db_pm",
        name="1:30PM MLB Prop DB Refresh",
        misfire_grace_time=900,
    )

    # 1:45 PM ET — MLB batter stats pre-fetch (before 2PM props run)
    scheduler.add_job(
        run_mlb_batter_stats,
        "cron",
        hour=13,
        minute=45,
        id="mlb_batter_stats_pm",
        name="1:45PM MLB Batter Stats Refresh",
        misfire_grace_time=900,
    )

    # 2:00 PM ET — Afternoon MLB props refresh
    scheduler.add_job(
        job_mlb_props_afternoon,
        "cron",
        hour=14,
        minute=0,
        id="mlb_props_afternoon",
        name="2PM Afternoon MLB Props",
        misfire_grace_time=1800,
    )
    ...
    scheduler.add_job(
        job_end_of_night_grader,
        "cron",
        hour=23,
        minute=0,
        id="end_night_grader",
        name="11PM End-of-Night Grader",
        misfire_grace_time=1800,
    )
